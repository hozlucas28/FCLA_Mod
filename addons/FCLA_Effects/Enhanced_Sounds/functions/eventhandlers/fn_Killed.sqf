...
_this spawn {
  ...
  if (!(alive vehicle _unit) && ((vehicle _unit isKindOf "Tank") || (vehicle _unit isKindOf "Wheeled_APC_F") || (vehicle _unit isKindOf "Helicopter"))) exitWith {
    ...
    [_firstSoundSourceObj, _randomSound, 7, true] spawn FCLA_Common_fnc_globalSay3D;
    ...
    for "_i" from 1 to 2 do {
      ...
      [_secondSoundSourceObj, _randomSound, 1, true] spawn FCLA_Common_fnc_globalSay3D;
    };
  };
  ...
  if ((_unit getHit "head") >= 0.4) exitWith {
    ...
    [_soundSourceObj, _randomSound, 5, true] spawn FCLA_Common_fnc_globalSay3D;
  };
  ...
  if ([_unit] call ACE_Common_fnc_isInBuilding) then {
    ...
    _soundSourceObj = createVehicle ["VirtualAISquad", getPos _unit, [], 0, "CAN_COLLIDE"];
    _soundSourceObj attachTo [_unit, [0, 0, 0.5], "Head"];
    [_soundSourceObj, _randomSound, 1, true] spawn FCLA_Common_fnc_globalSay3D;
  } else {
    ...
    _soundSourceObj = createVehicle ["VirtualAISquad", getPos _unit, [], 0, "CAN_COLLIDE"];
    _soundSourceObj attachTo [_unit, [0, 0, 0.5], "Head"];
    [_soundSourceObj, _randomSound, 9, true] spawn FCLA_Common_fnc_globalSay3D;
  };
};
