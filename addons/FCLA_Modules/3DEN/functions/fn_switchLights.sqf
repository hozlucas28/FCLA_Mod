...
//Asignar interruptor.
_findedTransferSwitch = if (_numberOfCompatibleSynchronizedObjects == 1) then {_compatibleSynchronizedObjects findIf {(typeOf _x) == "Land_TransferSwitch_01_F"};} else {-1;};
_transferSwitch = if (_findedTransferSwitch > -1) then {_compatibleSynchronizedObjects select _findedTransferSwitch;} else {_module;};
