...
if ((_ammoClass == "") || (_weaponClass == "") || (_minimumShots <= 0) || (_maximumShots <= 0) || (_minimumDelay <= 0) || (_maximumDelay <= 0) || ((count _compatibleSynchronizedObjects) != 1)) exitWith {["FCLA_Module_Ambient_Fired", "- MODULO: ASIGNAR DISPAROS AMBIENTALES", "¡Error! El/Un modulo 'Asignar disparos ambientales' no se pudo inicializar con exito."] call FCLA_Common_fnc_errorMessage;};
...
if ((_hasNotWeapons) || (_hasNotMagazines)) exitWith {["¡Error! El/Un modulo 'Asignar disparos ambientales' no se pudo inicializar porque el vehiculo no posee armas y/o municiones."] call FCLA_Common_fnc_errorMessage;};
