...
_repeatable = if (_distanceToShow > 0) then {_module getVariable ["FCLA_Repeatable", false];} else {false;};
...
if (_distanceToShow < 0) exitWith {
  ...
  }, [_module, _emitterObject, [[_emitter, _subtitle]], _color, _timeToHide, [_needShortRadio, _needLongRadio, _side, _distanceToShow]], 0.1] call CBA_fnc_waitAndExecute;
};
...
_trigger setVariable ["FCLA_Subtitle_Attributes", [_emitterObject, [[_emitter, _subtitle]], _color, _timeToHide, [_needShortRadio, _needLongRadio, _side, _distanceToShow]], true];
