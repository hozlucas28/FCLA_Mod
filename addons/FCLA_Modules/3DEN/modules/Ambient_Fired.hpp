...
class FCLA_Module_Ambient_Fired: Module_F {
  ...
  scope = 2;
	...
	class ModuleDescription: ModuleDescription {
		description[] =	{"Permite forzar los vehículos sincronizados al módulo a disparar hacia el cielo.<br/><br/>• El módulo puedo estar sincronizado a distintos vehículos, siempre y cuando estos sean iguales.<br/>• Si desea detener los disparos ambientales de un vehículo asignele la variable de tipo objeto 'FCLA_Ambient_Fire' con el valor 'true' ó 'nil'."};
	};
};
