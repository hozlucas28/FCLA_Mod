...
[missionNamespace,"arsenalOpened",{
	_this spawn {
...

		{
			...
			{
				_ctrl lbAdd _x ;
			} forEach ["Sort by rate of fire","Sort by accuracy","Sort by range","Sort by impact","Sort by weight"] ;
						...
						if (_data != "") then {
							_ctrl lbSetValue [_i,call {
								...
								if (_this#1 == 3) exitWith {
									...
									/*_value = 1000 ;
									{
										_value = _value min ((_wpnClass >> _x >> "dispersion") call BIS_fnc_parseNumberSafe) ;
									} forEach getArray (_wpnClass >> "modes") ;
									_value = log _value ;
									100000-_value*10000*/
								} ;
		...
		] ;
		{
			...
			{
				_ctrl lbAdd _x ;
			} forEach ["Sort by range","Sort by impact","Sort by weight"] ;
			_ctrl ctrlAddEventHandler ["LBSelChanged",{
				if (_this#1 >= 2) then {
					defineBase
					for "_i" from 0 to lbSize _ctrl-1 do {
		...
		] ;
		{
			...
			{
				_ctrl lbAdd _x ;
			} forEach ["Sort by ballistic protection","Sort by explosive resistance","Sort by load","Sort by weight"] ;
			_ctrl ctrlAddEventHandler ["LBSelChanged",{
				if (_this#1 >= 2) then {
					defineBase
					for "_i" from 0 to lbSize _ctrl-1 do {
...

		{
			...
			{
				_ctrl lbAdd _x ;
			} forEach ["Sort by load","Sort by weight"] ;
			_ctrl ctrlAddEventHandler ["LBSelChanged",{
				if (_this#1 >= 2) then {
					defineBase
					for "_i" from 0 to lbSize _ctrl-1 do {
						...
						if (ctrlFade _ctrl == 0) exitWith {\
							if (_i in [\
								...
								for "_i" from 0 to lbSize _ctrl-1 do {\
									...
									if ((_ctrl lbData _i == _curSel and !_found) or (_ctrl lbData _i == _selectedFound)) then {_ctrl lbSetCurSel _i} ;\
