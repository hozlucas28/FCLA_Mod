...
["Camera", {
  ...
  if (_inCameraMode) exitWith {[{["FCLA_System_Chat", ["Ya te encuentas utilizando el modo camara."]] call CBA_fnc_localEvent;}, [], 0.1] call CBA_fnc_waitAndExecute;};
  ...
  [format ["Mod FCLA - %1 [%2] entro al modo camara.", name _player, getPlayerUID _player]] call ACE_Common_fnc_serverLog;
  [{["FCLA_System_Chat", [format ["- Sistema: %1 entro al modo camara.", [_this] call FCLA_Common_fnc_getCleanName]]] call CBA_fnc_globalEvent;}, _player, 0.1] call CBA_fnc_waitAndExecute;

  [{!(_this in (call ACE_Spectator_fnc_players))}, {
    [format ["Mod FCLA - %1 [%2] salio del modo camara.", name _this, getPlayerUID _this]] call ACE_Common_fnc_serverLog;
    ["FCLA_System_Chat", [format ["- Sistema: %1 salio del modo camara.", [_this] call FCLA_Common_fnc_getCleanName]]] call CBA_fnc_globalEvent;
...
["Zeus", {
  ...
  if (_isNotAlive) exitWith {[{["FCLA_System_Chat", ["Debes estar vivo para utilizar este comando."]] call CBA_fnc_localEvent;}, [], 0.1] call CBA_fnc_waitAndExecute;};
  if (_inCameraMode) exitWith {[{["FCLA_System_Chat", ["No puedes utilizar este comando si te encuentras en el modo camara."]] call CBA_fnc_localEvent;}, [], 0.1] call CBA_fnc_waitAndExecute;};
  ...
  switch (_value) do {
    case "TRUE": {
      ...
      [_player, _player getVariable ["FCLA_Insignia", FCLA_Default_Insignia]] call FCLA_Interactions_fnc_statementInsignias; 
