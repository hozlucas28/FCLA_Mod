...
["Camera", {
  ...
  _cameraModeDisableByCurator = player getVariable ["FCLA_Camera_Mode_Disable_By_Curator", false];
  ...
  if (_cameraModeDisableByCurator) exitWith {[{systemChat "El Zeus te ha bloqueado el acceso al modo cámara.";}, [], 0.1] call CBA_fnc_waitAndExecute;};
...
["Zeus", {
  ...
  if (_isNotAlive) exitWith {[{systemChat "Debes estar vivo para utilizar este comando.";}, [], 0.1] call CBA_fnc_waitAndExecute;};
  if (_inCameraMode) exitWith {[{systemChat "No puedes utilizar este comando si te encuentras en el modo cámara.";}, [], 0.1] call CBA_fnc_waitAndExecute;};
