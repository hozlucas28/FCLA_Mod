...
/* ----------------------------------------------------------------------------
Function: FCLA_Development_fnc_initCameraMode
...
~ Click izquierdo: Alternar a la unidad
seleccionada.

~ Click derecho: Restablecer zoom.

~ 'L': Mostrar/Ocultar interfaz.

~ 'N': Visión nocturna/termica.

~ 'B': Alternar vista trasera del
objetivo.

~ 'F': Alternar modo de seguimiento.

~ 'C': Alternar enfoque.

~ 'T': Alternar modo de seguimiento
de proyectil.

~ 'V': Cambiar visión de la cámara.
...
~ Click izquierdo (mouse): Escanear y mostrar grupos cercanos.

~ Click izquierdo (en marcador): Alternar a la unidad  del marcador seleccionado.";

if (isNil "FCLA_Camera_Mode_Kill") then { FCLA_Camera_Mode_Kill = false };

FCLA_Camera_Mode_KD = [controlNull,-1,false,false,false];
FCLA_Camera_Mode_KU = [controlNull,-1,false,false,false];
FCLA_Camera_Mode_MD = [controlNull,-1,0.5,0.5,false,false,false];
FCLA_Camera_Mode_MU = [controlNull,-1,0.5,0.5,false,false,false];
FCLA_Camera_Mode_MM = [controlNull,0.0,0.0];
FCLA_Camera_Mode_MW = [controlNull,0];

FCLA_Camera_Mode_MC = false;
FCLA_Camera_Mode_MCP = [0.0,0.0,0.0];

FCLA_Camera_Mode_LSC = [0,-1];

FCLA_Camera_Mode_B = false;
FCLA_Camera_Mode_BId = "";

FCLA_Camera_Mode_T = false;
FCLA_Camera_Mode_Trigger_Fire = false;
FCLA_Camera_Mode_Trigger_Eject = false;
FCLA_Camera_Mode_O = objnull;

FCLA_Camera_Mode_S = false;
FCLA_Camera_Mode_X = 0.0;
FCLA_Camera_Mode_Y = 0.0;

FCLA_Camera_Mode_F = false;

#ifdef PARAMTRACK
	FCLA_Camera_Mode_Com1 = PARAMTRACKINITCODE1;
	FCLA_Camera_Mode_Com2 = PARAMTRACKINITCODE2;
	FCLA_Camera_Mode_Com3 = PARAMTRACKINITCODE3;
	FCLA_Camera_Mode_Com4 = PARAMTRACKINITCODE4;
#endif
...
_OpenList =
{
	...
	createDialog "FCLA_Camera_Mode_Dialog";
	waitUntil{ctrlShown(findDisplay 5100 displayCtrl 0)};

	lbAdd [ 2, "Libre visión" ];
	lbAdd [ 2, "1° persona (visión)" ];
	lbAdd [ 2, "3° persona (visión)" ];
	lbAdd [ 2, "Visión variable" ];
	lbSetCurSel [2, _vm];
	...
	onMapSingleClick "FCLA_Camera_Mode_MCP = _pos; FCLA_Camera_Mode_MC = true";

	#ifdef PARAMTRACK
		ctrlSetText[55,FCLA_Camera_Mode_Com1];
		ctrlSetText[56,FCLA_Camera_Mode_Com2];
		ctrlSetText[57,FCLA_Camera_Mode_Com3];
		ctrlSetText[58,FCLA_Camera_Mode_Com4];
	#endif
...
_CloseList =
{
	...
	deleteMarkerLocal "FCLA_Camera_Mode_c";
	{ deleteMarkerLocal _x } forEach _ma_mnl;
	{ deleteMarkerLocal _x } forEach _ma_gmnl;

	FCLA_Camera_Mode_LSC set [1,-1];
	FCLA_Camera_Mode_KD set [1,-1];
	FCLA_Camera_Mode_KU set [1,-1];
	FCLA_Camera_Mode_OC = [controlNull,0.0,0.0,false];
};
...
_ChangeModeFollow =
{
	...
	if (_fo) then
	{
		if (SHOWNOTICETEXT) then { titleText[ "<t size='1.5'>Modo de seguimiento - <t color='#00FF6B'>ACTIVADO</t>.</t>", "plain down", _w*15, true, true ] };
	...
	{
		if (SHOWNOTICETEXT) then { titleText[ "<t size='1.5'>Modo de seguimiento - <t color='#FF0000'>DESACTIVADO</t>.</t>", "plain down", _w*15, true, true ] };
...
_ChangeModeBehind =
{
	...
	if (_be) then
	{
		if (SHOWNOTICETEXT) then { titleText[ "<t size='1.5'>Vista trasera - <t color='#00FF6B'>ACTIVADO</t>.</t>", "plain down", _w*15, true, true ] };
	...
	{
		if (SHOWNOTICETEXT) then { titleText[ "<t size='1.5'>Vista trasera - <t color='#FF0000'>DESACTIVADO</t>.</t>", "plain down", _w*15, true, true ] };
...
_ChangeModeFocus =
{
	...
	if (_fc) then
	{
		if (SHOWNOTICETEXT) then { titleText[ "<t size='1.5'>Enfoque - <t color='#00FF6B'>ACTIVADO</t>.</t>", "plain down", _w*15, true, true ] };
	...
	{
		if (SHOWNOTICETEXT) then { titleText[ "<t size='1.5'>Enfoque - <t color='#FF0000'>DESACTIVADO</t>.</t>", "plain down", _w*15, true, true ] };
...
_ChangeModeTrigger =
{
	...
	if (_trg) then
	{
		if (SHOWNOTICETEXT) then { titleText[ "<t size='1.5'>Modo de seguimiento de proyectil - <t color='#00FF6B'>ACTIVADO</t>.</t>",  "plain down", _w*10, true, true ] };
	...
	{
		if (SHOWNOTICETEXT) then { titleText[ "<t size='1.5'>Modo de seguimiento de proyectil - <t color='#FF0000'>DESACTIVADO</t>.</t>",  "plain down", _w*10, true, true ] };
...
_SetFiredHandler =
{
	...
	[
		"Fired",
		{
			...
			{
				FCLA_Camera_Mode_O = nearestObject[_obj,_a];
				if (!(isNull FCLA_Camera_Mode_O)) then
				{
					FCLA_Camera_Mode_Trigger_Fire = true;
					FCLA_Camera_Mode_T = true;
				};
			};
		}
	];
	...
	[
		"GetOut",
		{
			FCLA_Camera_Mode_O = _this select 2;

			FCLA_Camera_Mode_Trigger_Eject = true;
			FCLA_Camera_Mode_T = true;
		}
	];
};
...
_CreateCameraMarker =
{
	createMarkerLocal["FCLA_Camera_Mode_c", [_cp select 0, _cp select 1]];
	"FCLA_Camera_Mode_c" setMarkerShapeLocal "ICON";
	"FCLA_Camera_Mode_c" setMarkerTypeLocal "mil_Arrow2";
	"FCLA_Camera_Mode_c" setMarkerColorLocal COLORICONCAMERA;
	"FCLA_Camera_Mode_c" setMarkerSizeLocal [0.27, 0.27];
	"FCLA_Camera_Mode_c" setMarkerDirLocal (-(_dr) + 90.0);
};
...
_CreateMapScanArea =
{
	[FCLA_Camera_Mode_MCP, _w] spawn
	{
		deleteMarkerLocal "FCLA_Camera_Mode_e";

		createMarkerLocal[ "FCLA_Camera_Mode_e", [ (_this select 0) select 0, (_this select 0) select 1 ] ];
		"FCLA_Camera_Mode_e" setMarkerShapeLocal "ELLIPSE";
		"FCLA_Camera_Mode_e" setMarkerBrushLocal "SOLID";
		"FCLA_Camera_Mode_e" setMarkerColorLocal COLORSCANAREA;
		"FCLA_Camera_Mode_e" setMarkerAlphaLocal 0.3;
		"FCLA_Camera_Mode_e" setMarkerSizeLocal [DETECTOBJECTDISTANCE, DETECTOBJECTDISTANCE];
...
_ClickSwitch =
{
	...
	FCLA_Camera_Mode_MD set [1, -1];
	FCLA_Camera_Mode_MU set [1, -1];
};
...
_SmoothMove =
{
	...
	while { ( [_sm_cp select 0, _sm_cp select 1] distance [_sm_cp_t_l select 0, _sm_cp_t_l select 1] ) > (_sm_d + (sqrt((velocity _o select 0)^2 + (velocity _o select 1)^2)*0.01)) and (_sm_n < 100) and _sm_l } do
	{
		...
		if (!(isNil "FCLA_Camera_Mode_KD")) then { _kd = FCLA_Camera_Mode_KD select 1 };
		if (!(isNil "FCLA_Camera_Mode_KU")) then { _ku = FCLA_Camera_Mode_KU select 1 };
		FCLA_Camera_Mode_KD set [1,-1];
		FCLA_Camera_Mode_KU set [1,-1];
		...
		if (!(isNil "FCLA_Camera_Mode_OC")) then { _oc = FCLA_Camera_Mode_OC select 3 };

		if (!(isNil "FCLA_Camera_Mode_MD")) then { _md = FCLA_Camera_Mode_MD select 1 };
		if (!(isNil "FCLA_Camera_Mode_MU")) then { _mu = FCLA_Camera_Mode_MU select 1 };
		...
		if (_ku == KEYMODEVIEW) then
		{
			...
			switch (_vm) do
			{
				case (0):
				{
					if (SHOWNOTICETEXT) then { titleText[ "<t size='1.5'>Libre visión.</t>", "plain down", _w*15, true, true  ] };

					lbSetCurSel [2, 0];
				};
				case (1):
				{
					if (SHOWNOTICETEXT) then { titleText[ "<t size='1.5'>Primera persona (visión).</t>", "plain down", _w*15, true, true  ] };

					lbSetCurSel [2, 1];
				};
				case (2):
				{
					if (SHOWNOTICETEXT) then { titleText[ "<t size='1.5'>Tercera persona (visión).</t>", "plain down", _w*15, true, true  ] };

					lbSetCurSel [2, 2];
				};
				case (3):
				{
					if (SHOWNOTICETEXT) then { titleText[ "<t size='1.5'>Visión variable.</t>", "plain down", _w*15, true, true ] };
		...
		if (_li) then
		{
			if (FCLA_Camera_Mode_B) then
			{
				switch (FCLA_Camera_Mode_BId) do
				{
					case ("fo"):
					{
						call _ChangeModeFollow;
			...
			if (!(isNil "FCLA_Camera_Mode_MD2")) then { _ma_md = FCLA_Camera_Mode_MD2 };
			if (_ma_md select 1 == 0) then
			{
				...
				FCLA_Camera_Mode_MD2 = [controlNull,-1,0.0,0.0,false,false,false];
				FCLA_Camera_Mode_MU2 = [controlNull,-1,0.0,0.0,false,false,false];
			...
			if (_ma_dr) then
			{
				(findDisplay 5100 displayCtrl 22) ctrlSetPosition
				[
					(_ma_pb select 0) + (FCLA_Camera_Mode_OC select 1) - _ma_pm,
					_ma_pb select 1
				];
				(findDisplay 5100 displayCtrl 22) ctrlCommit 0.0;
				...
				[
					_ma_pd select 0,
					(_ma_pd select 1) - ((FCLA_Camera_Mode_OC select 1) - _ma_pm),
					(_ma_pd select 2) + (FCLA_Camera_Mode_OC select 1) - _ma_pm,
					(_ma_pd select 3) + (FCLA_Camera_Mode_OC select 1) - _ma_pm
				];
				(findDisplay 5100 displayCtrl 100) ctrlCommit 0.0;
				...
				if (!(isNil "FCLA_Camera_Mode_MU2")) then { _ma_mu = FCLA_Camera_Mode_MU2 select 1 };
				if ( _ma_mu == 0 or !(FCLA_Camera_Mode_OC select 3) ) then { _ma_dr = false };
			};


			"FCLA_Camera_Mode_c" setMarkerPosLocal [_sm_cp select 0, _sm_cp select 1];
			"FCLA_Camera_Mode_c" setMarkerDirLocal (-(_dr) + 90.0);
			...
			if ( alive _o ) then
			{
				"FCLA_Camera_Mode_o" setMarkerPosLocal [_ma_op select 0, _ma_op select 1];
				if ((speed vehicle _o) > 4.0) then
				{
					"FCLA_Camera_Mode_o" setMarkerTypeLocal "mil_Arrow2";
					"FCLA_Camera_Mode_o" setMarkerSizeLocal [0.27, 0.27];
					"FCLA_Camera_Mode_o" setMarkerDirLocal (getdir vehicle _o);
				}
				else
				{
					"FCLA_Camera_Mode_o" setMarkerTypeLocal "mil_dot";
					"FCLA_Camera_Mode_o" setMarkerSizeLocal [0.65, 0.65];
				};
			}
			else
			{
				"FCLA_Camera_Mode_o" setMarkerSizeLocal [0.0, 0.0];
				"FCLA_Camera_Mode_o" setMarkerTextLocal "";
			};
...
_ChaseMissile =
{
	...
	while {(alive _cm_o) and _cm_l} do
	{
		...
		_mm = [ FCLA_Camera_Mode_MM select 1, FCLA_Camera_Mode_MM select 2 ];
		_acm = accTime^1.5 + 0.007;
		...
		_c camSetTarget [ (_cm_cp select 0) + (cos(_dr+FCLA_Camera_Mode_X)) * (cos(_dv+FCLA_Camera_Mode_Y)) * 100000.0, (_cm_cp select 1) + (sin(_dr+FCLA_Camera_Mode_X)) * (cos(_dv+FCLA_Camera_Mode_Y)) * 100000.0, (_cm_cp select 2) + (sin(_dv+FCLA_Camera_Mode_Y)) * 100000.0];
		_c camCommit 0;
...
_ehid_keydown = (findDisplay 46) displayAddEventHandler ["KeyDown", "FCLA_Camera_Mode_KD = _this"];
_ehid_keyup = (findDisplay 46) displayAddEventHandler ["KeyUp", "FCLA_Camera_Mode_KU = _this"];
_ehid_mousemove = (findDisplay 46) displayAddEventHandler ["MouseMoving", "FCLA_Camera_Mode_MM = _this"];
_ehid_mousezchange = (findDisplay 46) displayAddEventHandler ["MouseZChanged", "FCLA_Camera_Mode_MW = _this"];
_ehid_mousebd = (findDisplay 46) displayAddEventHandler ["MouseButtonDown", "FCLA_Camera_Mode_MD = _this"];
_ehid_mousebu = (findDisplay 46) displayAddEventHandler ["MouseButtonUp", "FCLA_Camera_Mode_MU = _this"];
...
while {_l} do
{
	...
	if (FCLA_Camera_Mode_T) then
	{
		FCLA_Camera_Mode_T = false;

		if (_fo and _vm == 0) then
		{
			if (FCLA_Camera_Mode_Trigger_Fire) then
			{
				if (CHASECAMERASHAKE) then
				{
					[ 15, 73, 112, 0.2, 13, 71, 118, 0.2, 0.04 ] spawn
					{
						...
						FCLA_Camera_Mode_S = true;
						while { FCLA_Camera_Mode_S } do
						{
							FCLA_Camera_Mode_X = FCLA_Camera_Mode_X + ((cos _a1) + (cos _a2)/1.5 + (cos _a3)/2) * _s1;
							FCLA_Camera_Mode_Y = FCLA_Camera_Mode_Y + ((cos _a4) + (cos _a5)/1.5 + (cos _a6)/2) * _s2;
			...
			if (FCLA_Camera_Mode_Trigger_Eject) then
			{
				FCLA_Camera_Mode_Trigger_Eject = false;

				_o = vehicle FCLA_Camera_Mode_O;
	...
	if (_md == 0 and _mu != 0 and !(_cs_m)) then
	{
		...
		if (!(FCLA_Camera_Mode_F)) then { ctrlSetFocus (findDisplay 5100 displayCtrl 100) };
	};
	if ( (_mu == 0) or (_mu == 1 and _md == 1) ) then
	{
		_cs_m = false;
		...
		if (!(isNil "FCLA_Camera_Mode_OCL")) then { _ocl = FCLA_Camera_Mode_OCL select 3 };
	...
	if (!(_li) and _vm == 0) then
	{
		...
		FCLA_Camera_Mode_MM set [ 1, 0.0 ];
		FCLA_Camera_Mode_MM set [ 2, 0.0 ];
	};
	...
	if (_li) then
	{
		...
		if (!(isNil "_lsc")) then { _lsc = FCLA_Camera_Mode_LSC select 1 };
		if ( _lsc != -1 ) then
		{
			sleep (_w*15);
		...
		if (FCLA_Camera_Mode_MC) then
		{
			_ma_st = diag_tickTime;

			_ma_ol = [ nearestObjects [[FCLA_Camera_Mode_MCP select 0, FCLA_Camera_Mode_MCP select 1, (visiblePosition _c select 2)+50], ["AllVehicles"], DETECTOBJECTDISTANCE] ] call _SelectObjects;
			...
			if (_ma_sc) then
			{
				...
				(
					(count _ma_ol) > 0
					and
					((findDisplay 5100 displayCtrl 100) posWorldToScreen FCLA_Camera_Mode_MCP) distance ((findDisplay 5100 displayCtrl 100) posWorldToScreen (getMarkerPos format["%1g",(_ma_ol select 0)])) < 0.02
				)
				then
				{
					lbClear 50;
			...
			else
			{
				...
				(
					...
					and
					((findDisplay 5100 displayCtrl 100) posWorldToScreen FCLA_Camera_Mode_MCP) distance ((findDisplay 5100 displayCtrl 100) posWorldToScreen (getMarkerPos format["%1%2%3",typeOf(driver vehicle(_ma_ol select 0)), rankId(driver vehicle(_ma_ol select 0)), name(driver vehicle(_ma_ol select 0))])) < 0.04
				)
				then
				{
					_ma_sc = true;
		...
		if (!(isNil "FCLA_Camera_Mode_OCM")) then { _ocm = FCLA_Camera_Mode_OCM select 3 };
		...
		{
			"FCLA_Camera_Mode_c" setMarkerPosLocal [_cp select 0, _cp select 1];
			"FCLA_Camera_Mode_c" setMarkerDirLocal (-(_dr) + 90.0);
...
<t color='#ff8a8a'>combatMode</t>: %29 <t color='#ff8a8a'>formation</t>: %30 <t color='#ff8a8a'>speedMode</t>: %31",
	...
	if ( (!(LIMITQUIT) and _ku == KEYQUIT) or !(alive player) or FCLA_Camera_Mode_Kill) then
	{
		if ( !(alive player) or FCLA_Camera_Mode_Kill ) then { _quit = true };

		if ( _ku == KEYQUIT ) then
		{
			setMousePosition [0.500,0.575];

			createDialog "FCLA_Camera_Mode_Dialog_Quit";
